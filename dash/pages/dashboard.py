...
import requests
import json
from components import tree_view
    ...
    return html.Div([           
        ...
        dbc.Row([
                ...
                # dbc.Collapse(
                    # dbc.Col([
                    # ], width=3),
                    #     dbc.Col(html.Div("Content to be collapsed"), width=3),  # Replace with your actual content
                    #     id="collapse",
                    #     is_open=False,
                    # ),
...
def update_sash_graph(date):
    ...
    final_df.index = final_df.index.map(lambda x: x.to_pydatetime().replace(
        tzinfo=tz.tzutc()).astimezone(tz.tzlocal()))
    final_df_long = pd.melt(final_df, value_vars=[
        "occ", "unocc"], ignore_index=False)
    ...
    sash_fig = px.bar(final_df_long, x=final_df_long.index, y="value", color="variable",
                      ...
                      color_discrete_map={
                          'occ': 'mediumseagreen', 'unocc': '#d62728'},
                      hover_data={"variable": True, "value": False},
                      custom_data=['variable']
                      )

    sash_fig.update_traces(hovertemplate=(
        'The fume hood was open for %{value} minutes when %{customdata}'))
...
def update_energy_graph(date):
    ...
    energy_fig.update_traces(hovertemplate=(
        'The fume hood used %{value} BTUs of energy when %{customdata}'))
